...
<body>
  ...
  <script>
      ...
      while (Date.now() - startedAt < timeoutMs) {{
        try {{
          ...
          if (state.last_scraped_at && state.last_scraped_at !== previousLastScrapedAt) {{
            ...
            document.getElementById("scrape-last-ago").textContent = `${{formatHoursAgo(state.last_scraped_at)}} · 마지막 모드 ${{state.mode || 'collect'}}`;
            document.getElementById("runtime-status").textContent = "수동 실행이 완료됐습니다. 수집된 공고는 즉시 텔레그램 발송 경로로 넘어갑니다.";
            return;
          }}
        }} catch (error) {{
    ...
    const runScrapeNow = async () => {{
      ...
      document.getElementById("runtime-status").textContent = "수동 실행을 시작했습니다. 완료되면 수집된 공고를 바로 텔레그램으로 보냅니다.";
      try {{
