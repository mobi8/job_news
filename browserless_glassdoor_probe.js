...
function buildBrowserlessQueryParams({ solveCaptchas = false } = {}) {
...
function buildBrowserlessUrls({ solveCaptchas = false } = {}) {
  const queryParams = buildBrowserlessQueryParams({ solveCaptchas });
  const queryString = queryParams.toString();
  const token = queryParams.get('token') || '';
  const baseUrls = [];
  if (process.env.BROWSERLESS_WS_URL) {
    baseUrls.push(process.env.BROWSERLESS_WS_URL);
  }

  const region = process.env.BROWSERLESS_REGION;
  if (region) {
    baseUrls.push(`wss://${region}`);
  } else {
    baseUrls.push(
  ...
  for (const baseUrl of baseUrls) {
    const normalized = baseUrl.includes('?') ? baseUrl : `${baseUrl}${baseUrl.endsWith('/') ? '' : ''}`;
    candidates.push(
      `${normalized}${normalized.includes('?') ? '&' : '?'}${queryString}`,
      `${normalized.replace(/\/$/, '')}/stealth?${queryString}`,
      `${normalized.replace(/\/$/, '')}/chromium/stealth?${queryString}`,
      `${normalized.replace(/\/$/, '')}/chrome/stealth?${queryString}`
    );
  }
...
async function main() {
  ...
  async function runWithMode({ solveCaptchas }) {
    const browserlessUrls = buildBrowserlessUrls({ solveCaptchas });
    let browser = null;
    let lastError = null;

    for (const browserlessUrl of browserlessUrls) {
      try {
        progress(`Glassdoor | connect ${browserlessUrl.split('?')[0]} | captcha=${solveCaptchas ? 'on' : 'off'}`);
        browser = await chromium.connectOverCDP(browserlessUrl);
        break;
      } catch (error) {
        lastError = error;
        progress(`Glassdoor | connect failed ${browserlessUrl.split('?')[0]} | ${error.message || error}`);
      }
    }

    if (!browser) {
      throw lastError || new Error('Unable to connect to Browserless.');
    }
